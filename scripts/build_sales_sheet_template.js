...
const fieldMapRows = [
  ...
  ['Synced At', 'sheet_writer runtime', 'Time when row is appended/updated'],
  ['Finished At', 'investigation_jobs.finished_at', 'Investigation completion time'],
  ['Priority', 'confidence_score', 'Hot >= 75, Warm 60-74'],
  ['Sales Status', 'sheet default/manual', 'Default New; sales edits manually'],
  ['Prospect Name', 'brand_name/business_name/full_name/email', 'Pick best sales-facing display name'],
  ['Email', 'investigation_jobs.email', 'Register email'],
  ['Phone', 'register_intake_jobs.payload_json.no_hp', 'If available from register payload'],
  ['Brand', 'investigation_jobs.brand_name', 'Register brand field'],
  ['Website', 'investigation_jobs.business_website', 'Only if clean URL is available'],
  ['Source Register', 'investigation_jobs.source', 'webhook/platform/manual source'],
  ['Owner Sales', 'manual', 'Filled by sales team'],
  ['Next Follow Up', 'manual', 'Filled by sales team'],
  ['Sales Notes', 'manual', 'Filled by sales team'],
  ['Dashboard Detail', 'job id + dashboard base URL', 'Link to full internal detail page'],
  ['Job ID', 'investigation_jobs.id', 'Hidden/internal reference'],
  ['Filter rule', 'classification/confidence', 'Only possible_company_affiliated with confidence >= 60'],
  ['Do not sync', 'internal data', 'No token/cost/raw evidence/tool logs/reasoning text'],
];
...
const sheets = [
  {
    ...
    widths: [18, 18, 16, 18, 24, 30, 18, 20, 28, 18, 18, 18, 34, 46, 38],
    freeze: true,
    filter: `A1:O${Math.max(200, sampleRows.length + 1)}`,
    dataValidations: [
      { range: 'C2:C500', formula: "'Options'!$A$2:$A$3" },
      { range: 'D2:D500', formula: "'Options'!$B$2:$B$7" },
    ],
  },
  {
    name: 'Options',
  ...
  },
  {
    ...
    widths: [22, 36, 70],
    freeze: true,
    filter: `A1:C${fieldMapRows.length}`,
  },
];
