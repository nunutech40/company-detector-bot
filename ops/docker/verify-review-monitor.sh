...
for key in GBP_BUSINESS_NAME GBP_ACCOUNT_ID GBP_LOCATION_ID GBP_CLIENT_ID GBP_CLIENT_SECRET GBP_REFRESH_TOKEN REVIEW_MONITOR_SLACK_CHANNEL; do
  [[ -n "$(env_value "$key")" ]] || fail "${key} is missing in ${ENV_FILE}"
done
grep -Eq '^SLACK_BOT_TOKEN=.+$' .env || fail "SLACK_BOT_TOKEN is missing in core .env"
pass "Required Google Business Profile and Slack values configured"
...
pass "Slack test-send succeeded"
